...
async function* runAgentInGraph(
  ...
  try {
    ...
    const artifactTools = createArtifactTools({ artifacts });
    ...
    const effectiveModel = resolveModel(
      {
        ...
        spentUsd: Object.values(artifacts)
          .filter((v) => v.startsWith("__cost__"))
          .reduce((s, v) => s + parseFloat(v.slice(8) || "0"), 0),
    ...
    if (worktreePath) {
      artifacts[`${agent.name}-worktree`] = worktreePath;
    }
...
export async function* runSwarmGraph(config: SwarmConfig): AsyncGenerator<SwarmEvent> {
  ...
  for (let waveIdx = startWave; waveIdx < waves.length; waveIdx++) {
    ...
    // Run all agents in this wave concurrently, merging their event streams
    const gens = wave.map((agent) =>
      runAgentInGraph(agent, config, artifacts, swarmId),
    );
